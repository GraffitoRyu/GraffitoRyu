...
10. Schema v3 snapshots and envelopes contain no source identity or stable digest. Bind them only to the source slot selected by the installed private config and fixed receiver route; never serialize that binding. Match v3 acknowledgements by revision and detect same-revision conflicts by canonical snapshot equality.
