...
8. Treat account usage as one separate publisher-side sample. Validate only observation time, usage window, used percentage, reset time, rate-limit state, credit availability/unlimited flags, and coverage. Never place account usage in a device snapshot, envelope, diagnostic, or public activity artifact.
