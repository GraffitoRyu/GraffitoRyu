...
9. On every Git retry, rebuild from the newest remote branch. Write the owned collection first, then read both canonical collections from that candidate. Missing, wrong-window, malformed, or unavailable inputs preserve the existing final JSON and SVG.
10. The Git allowlist is the owned collection plus the two final generated files. Never stage the peer collection, README, source, private state, or config. Never force-push.
