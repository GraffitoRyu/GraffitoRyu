...
- Each installed device publisher may update only its owned collection file and may replace `metrics/codex-activity.json` and `assets/codex-activity.svg` only after both public collections validate for the current KST window. Automated runs must not edit README, the peer collection, or source files.
