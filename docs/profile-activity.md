...
Schema v3 snapshot and envelope bodies contain no source identity. The installed private config and fixed receiver route attach a transient source slot only in memory for validation and aggregation; that routing identity is never serialized into the activity surface.
