...
Verified account activity is a second sanitized input. It accepts only an exact 30-day Analytics window with daily turns, plugin calls, and skill uses, plus one rounded profile token display. The contract records whether each UI read succeeded, whether refresh cadence is exposed, whether collection is automated, and whether the field is comparable to local telemetry. It rejects identifiers, names, paths, credentials, raw responses, and arbitrary fields. The installed runtime stores the validated sample in private state, joins it once after the two local device collections, and publishes only the anonymous fixed-schema values.

Account Analytics values remain separate from local telemetry unless both period and definition match. A comparable account value replaces a missing, zero, or smaller local value. A larger local value is a contradiction and stops publication. The current Analytics turns, plugin calls, and skill uses use different definitions from local sessions and tool telemetry, so the dashboard shows them as separate account metrics. The rounded profile token display also remains separate because its timezone, included surfaces, exact value, and refresh cadence are not exposed.
