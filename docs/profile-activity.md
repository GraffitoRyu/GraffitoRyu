...
Schema v3 snapshot and envelope bodies contain no source identity or stable digest. The installed private config and fixed receiver route attach a transient source slot only in memory for validation and aggregation; that routing identity is never serialized into the activity surface. Schema v3 acknowledgements use the revision only, and same-revision conflicts compare canonical snapshot values directly. Schema v2 retains its digest contract for compatibility.

The Mac mini receiver validates the version-matched envelope contract, configured source route, revision ordering, and conflict rules before atomically replacing last-good. It never derives correctness from task memory. A successful receive after 08:00 KST is followed by the same gated `run` command used by the daily 08:00 task. `run` processes the two stored snapshots; it does not collect either device.
